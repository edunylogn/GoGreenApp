                    ...
                    <?php foreach ($roomSizes as $j => $size){ ?>
                        <td>
                            <div class="input-group input-group-sm">
                                ...
                                <input type="text" class="form-control" name="room_price_details[<?=$id + $j?>][price]" value="<?=$roomPriceDetails[$id + $j]['price']?>">
                                <input type="hidden" name="room_price_details[<?=$id + $j?>][id]" value=<?=$roomPriceDetails[$id + $j]['id']?>>
                            </div>
                        </td>
                    <?php } ?>
                    <td>
                    ...
                    <?php foreach ($roomSizes as $j => $size){ ?>
                        <td>
                            <div class="input-group input-group-sm <?= isset($field['date']) && $field['date']? 'date datetimepicker-field':'' ?>">
                                ...
                                <input type="<?= $field['type'] ?>" class="form-control" name="room_price_details[<?=$id + $j?>][<?=$key?>]" value="<?= $roomPriceDetails[$id + $j][$key]?>">
                            </div>
                        </td>
                    <?php } ?>
                        <td></td>
